...
LIBFREENECT2_MAYBE_UNUSED size_t
...
LIBFREENECT2_MAYBE_UNUSED uint32_t

Frame::timestamp() const {
    return frame->timestamp;
}

LIBFREENECT2_MAYBE_UNUSED uint32_t

Frame::sequence() const {
...
LIBFREENECT2_MAYBE_UNUSED FrameFormat
...
public:
    ...
    rust::Fn<
    void(FrameType, std::unique_ptr<Frame>,
    ...
    bool onNewFrame(libfreenect2::Frame::Type type, libfreenect2::Frame *frame) override {
        on_new_frame(static_cast<FrameType>(type), std::make_unique<Frame>(frame), ctx);
        return true;
    }
...
libfreenect2_ffi::create_frame_listener(rust::cxxbridge1::Box<CallContext>
ctx,
rust::Fn<
void(FrameType, std::unique_ptr<Frame>,
const rust::cxxbridge1::Box <CallContext> &)> on_new_frame) {
